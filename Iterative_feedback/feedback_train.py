...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "The path to the attribute reward model."),
    )
    parser.add_argument(
        "--reward_spatial_path",
        type=str,
        required=True,
        help=(
            "The path to the spatial reward model."),
    )
    parser.add_argument(
        ...
        required=True,
        help=(
            "The path to the nonspatial reward model."),
    )
...
def encode_prompt_rm(batch, caption_column, reward_model_attribute, reward_model_spatial, reward_model_nonspatial):
    prompt_batch = batch[caption_column]

    text_inputs_attribute = reward_model_attribute.blip.tokenizer(prompt_batch, padding='max_length', truncation=True, max_length=35, return_tensors="pt")
    text_inputs_spatial = reward_model_spatial.blip.tokenizer(prompt_batch, padding='max_length', truncation=True, max_length=35, return_tensors="pt")
    text_inputs_nonspatial = reward_model_nonspatial.blip.tokenizer(prompt_batch, padding='max_length', truncation=True, max_length=35, return_tensors="pt")


    return {
        "rm_input_ids_attribute": text_inputs_attribute.input_ids,
        "rm_attention_mask_attribute": text_inputs_attribute.attention_mask,
        "rm_input_ids_spatial": text_inputs_spatial.input_ids,
        "rm_attention_mask_spatial": text_inputs_spatial.attention_mask,
        "rm_input_ids_nonspatial": text_inputs_nonspatial.input_ids,
        "rm_attention_mask_nonspatial": text_inputs_nonspatial.attention_mask,
    }
...
class Trainer(object):
    def __init__(self, pretrained_model_name_or_path, train_data_dir, args) -> None:
        ...
        self.reward_model_attribute = RM.ImageReward(device='cuda', med_config='train/config/med_config.json').to('cuda')
        my_state_dict_attribute = torch.load(args.reward_attribute_path, map_location='cpu')
        my_state_dict_attribute = {key.split('module.')[1]: value for key, value in my_state_dict_attribute.items()}
        self.reward_model_attribute.load_state_dict(my_state_dict_attribute,strict=False)

        self.reward_model_spatial = RM.ImageReward(device='cuda', med_config='train/config/med_config.json').to('cuda')
        my_state_dict_spatial = torch.load(args.reward_spatial_path, map_location='cpu')
        my_state_dict_spatial = {key.split('module.')[1]: value for key, value in my_state_dict_spatial.items()}
        self.reward_model_spatial.load_state_dict(my_state_dict_spatial,strict=False)

        self.reward_model_nonspatial = RM.ImageReward(device='cuda', med_config='train/config/med_config.json').to('cuda')
        my_state_dict_nonspatial = torch.load(args.reward_nonspatial_path, map_location='cpu')
        my_state_dict_nonspatial = {key.split('module.')[1]: value for key, value in my_state_dict_nonspatial.items()}
        self.reward_model_nonspatial.load_state_dict(my_state_dict_nonspatial,strict=False)

        # Freeze vae and text encoders.
        self.vae.requires_grad_(False)
        text_encoder_one.requires_grad_(False)
        text_encoder_two.requires_grad_(False)
        self.reward_model_attribute.requires_grad_(False)
        self.reward_model_spatial.requires_grad_(False) 
        self.reward_model_nonspatial.requires_grad_(False)
        ...
        self.reward_model_attribute.to(self.accelerator.device, dtype=self.weight_dtype)
        self.reward_model_spatial.to(self.accelerator.device, dtype=self.weight_dtype)
        self.reward_model_nonspatial.to(self.accelerator.device, dtype=self.weight_dtype)
        ...
        def preprocess_train(examples):
            original_sizes = []
            crop_top_lefts = []
            num_data = len(examples[image_column])
            for img in range(num_data):
                original_sizes.append((512, 512))
        ...
        compute_rm_encodings_fn = functools.partial(
            encode_prompt_rm, 
            caption_column=args.caption_column, 
            reward_model_attribute=self.reward_model_attribute,
            reward_model_spatial=self.reward_model_spatial,
            reward_model_nonspatial=self.reward_model_nonspatial
        )
        ...
        with self.accelerator.main_process_first():
            ...
            self.precomputed_dataset = concatenate_datasets(
                ...
                #  train_dataset_with_vae.remove_columns(["image", "text", "id"]),
                 train_dataset_with_rm.remove_columns(["image", "text"])], axis=1
            )
            self.precomputed_dataset = self.precomputed_dataset.with_transform(preprocess_train)

        # del compute_vae_encodings_fn, compute_embeddings_fn, text_encoder_one, text_encoder_two
        del compute_embeddings_fn, text_encoder_one, text_encoder_two
        ...
        def collate_fn(examples):
            ...
            rm_input_ids_attribute = torch.stack([torch.tensor(example["rm_input_ids_attribute"]) for example in examples])
            rm_attention_mask_attribute = torch.stack([torch.tensor(example["rm_attention_mask_attribute"]) for example in examples])
            rm_input_ids_attribute = rm_input_ids_attribute.view(-1, rm_input_ids_attribute.shape[-1])
            rm_attention_mask_attribute = rm_attention_mask_attribute.view(-1, rm_attention_mask_attribute.shape[-1])

            rm_input_ids_spatial = torch.stack([torch.tensor(example["rm_input_ids_spatial"]) for example in examples])
            rm_attention_mask_spatial = torch.stack([torch.tensor(example["rm_attention_mask_spatial"]) for example in examples])
            rm_input_ids_spatial = rm_input_ids_spatial.view(-1, rm_input_ids_spatial.shape[-1])
            rm_attention_mask_spatial = rm_attention_mask_spatial.view(-1, rm_attention_mask_spatial.shape[-1])

            rm_input_ids_nonspatial = torch.stack([torch.tensor(example["rm_input_ids_nonspatial"]) for example in examples])
            rm_attention_mask_nonspatial = torch.stack([torch.tensor(example["rm_attention_mask_nonspatial"]) for example in examples])
            rm_input_ids_nonspatial = rm_input_ids_nonspatial.view(-1, rm_input_ids_nonspatial.shape[-1])
            rm_attention_mask_nonspatial = rm_attention_mask_nonspatial.view(-1, rm_attention_mask_nonspatial.shape[-1])
            ...
            return {
                ...
                "rm_input_ids_attribute": rm_input_ids_attribute,
                "rm_attention_mask_attribute": rm_attention_mask_attribute,
                "rm_input_ids_spatial": rm_input_ids_spatial,
                "rm_attention_mask_spatial": rm_attention_mask_spatial,
                "rm_input_ids_nonspatial": rm_input_ids_nonspatial,
                "rm_attention_mask_nonspatial": rm_attention_mask_nonspatial,
            }
    ...
    def _reward_loss(self, args, batch) -> torch.Tensor:
        ...
        image = self.vae.decode(pred_original_sample.to(torch.float32)).sample
        ...
        rewards_attribute = self.reward_model_attribute.score_gard(batch["rm_input_ids_attribute"], batch["rm_attention_mask_attribute"], image)
        rewards_spatial = self.reward_model_spatial.score_gard(batch["rm_input_ids_spatial"], batch["rm_attention_mask_spatial"], image)
        rewards_nonspatial = self.reward_model_nonspatial.score_gard(batch["rm_input_ids_nonspatial"], batch["rm_attention_mask_nonspatial"], image)
        
        loss = F.relu(-rewards_attribute+2) + F.relu(-rewards_spatial+2) + F.relu(-rewards_nonspatial+2)
